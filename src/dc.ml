			...
			List.iter (fun state_label ->
				...
				let do_it trans =
					List.iter (fun (what,into) ->
						let get_lse s =
							match s with
							Alter_Trans_String s ->
								begin
									if s = "" then (Dlg.Local_String({ lse_male = s ; lse_male_sound = "";
						...
						| _ -> failwith(Printf.sprintf "DCUnknown ALTER_TRANS command: %s" what)
					) changes;
				in
				match transition_indices with
				| [] -> (* do them all *)
  ...
  | Chain3(ci) -> begin
      if not ci.c3_append_all then begin
        ...
        let ci = {ci with c3_dialogue = (
          ...
          List.iter (fun x -> if not x.c3du_ifexists then new_dialogue := x :: !new_dialogue
          else begin
        ...
        | hd :: tl ,true ->
          ...
          | hd' :: tl' ->
              ...
              | None -> "\"\""
              in
              Modder.handle_msg "ICT2_ACTIONS" (Printf.sprintf
...
let preprocess_ife game a =
  ...
  | Create(d) ->
      Array.iter (fun s ->
        s.Dlg.trans <- prife game s.Dlg.trans
      ) d.Dlg.state ;
      Create(d)
  ...
  | Append(s,unsafe,sl) ->
      ...
      Append(s,unsafe,sl)
  | Extend_Top(a,b,pos,tl) ->    Extend_Top(a,b,pos,
    Array.to_list (prife game (Array.of_list tl)))
  | Extend_Bottom(a,b,pos,tl) -> Extend_Bottom(a,b,pos,
    Array.to_list (prife game (Array.of_list tl)))
  | Replace(n,sl) ->
      List.iter (fun s -> s.Dlg.trans <- prife game s.Dlg.trans) sl ;
