...
let rec eval_pe buff game p =
  ...
  | PE_GameIs(game_list,isGameCheck) -> begin
      ...
      let res = List.exists (fun this ->
        ...
        | "EET"        -> eet
        | "SOD" -> sod && not eet
        | _ -> log_and_print "WARNING: No rule to identify %s" (String.uppercase this)
      ) game_list in
  ...
  | PE_GameIncludes(game_set) -> begin
      ...
      | "CA" -> ca
      | "SOD" -> sod
      | _ -> log_and_print "WARNING: GAME_INCLUDES has no rule for %s" (String.uppercase game_set)) in
