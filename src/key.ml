...
let key_of_ext warn ext = 
  ...
  with e -> 
    if warn then log_and_print "ERROR: unknown resource extension [%s]\n" ext ;
    raise e

let save_key key outchan = begin
  ...
  Stats.time "marshal KEY" (fun () -> 
    ...
    Array.iter (fun b ->
      ...
      let filename_offset = base_name_offset + (Buffer.length biff_name_buff) in
...
let bif_of_resource key name ext = begin
...
let bif_exists_in_key key name = begin
  let result = ref false in
  let name = Str.global_replace (Str.regexp "[\\/:]") Arch.biff_path_separator name in
  Array.iter (fun b ->
