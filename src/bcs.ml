...
let invalid_for_ict2 (al: string) =
	let al = !parse_al al in
	List.exists (fun action -> match (Load.the_game()).Load.script_style, Int32.to_int action.action_id with
	| _, 96  (* Rest() *)
	| _, 120 (* StartCutScene() *)
...
let invalid_for_ict1 (al: string list) = 
	let al = String.concat "" al in
	let al = !parse_al al in
	List.exists (fun action -> match (Load.the_game()).Load.script_style, Int32.to_int action.action_id with
	| _, 0   (* NoAction() *)
	...
	| _, 7   (* CreateCreature(S:NewObject,P:Location,I:Face) *)
	...
	| _, 49   (* MoveViewPoint(P:Target,I:ScrollSpeedScroll) *)
	| _, 50   (* MoveViewObject(O:Target,I:ScrollSpeedScroll) *)
	| _, 51   (* ClickLButtonPoint(P:Target,I:ScrollSpeedScroll) *)
	| _, 52   (* ClickLButtonObject(O:Target,I:ScrollSpeedScroll) *)
	| _, 53   (* ClickRButtonPoint(P:Target,I:ScrollSpeedScroll) *)
	| _, 54   (* ClickRButtonObject(O:Target,I:ScrollSpeedScroll) *)
	| _, 55   (* DoubleClickLButtonPoint(P:Target,I:ScrollSpeedScroll) *)
	| _, 56   (* DoubleClickLButtonObject(O:Target,I:ScrollSpeedScroll) *)
	| _, 57   (* DoubleClickRButtonPoint(P:Target,I:ScrollSpeedScroll) *)
	| _, 58   (* DoubleClickRButtonObject(O:Target,I:ScrollSpeedScroll) *)
	| _, 59   (* MoveCursorPoint(P:Target,I:ScrollSpeedScroll) *)
	...
	| _, 64   (* UndoExplore() *)
	| _, 65   (* Explore() *)
	| _, 66   (* DayNight(I:TimeOfDayTime) *)
	| _, 67   (* Weather(I:WeatherWeather) *)
	...
	| _, 119   (* DropInventory() *)
	| _, 120   (* StartCutScene(S:CutScene) *)
	| _, 121   (* StartCutSceneMode() *)
	| _, 122   (* EndCutSceneMode() *)
	| _, 123   (* ClearAllActions() *)
	| _, 124   (* Berserk() *)
	...
	| _, 142   (* UseDoor() *)
	| _, 143   (* OpenDoor(O:Object) *)
	| _, 144   (* CloseDoor(O:Object) *)
	| _, 145   (* PickLock(O:Object) *)
	| _, 146   (* Polymorph(I:AnimationTypeAnimate) *)
	| _, 147   (* RemoveSpell(I:SpellSpell) *)
	| _, 148   (* BashDoor(0:Object) *)
	...
	| _, 161   (* IncrementChapter(S:RESREF) *)
	| _, 162   (* ReputationSet(I:Reputation) *)
	| _, 163   (* ReputationInc(I:Reputation) *)
	| _, 164   (* AddexperienceParty(I:XP) *)
	| _, 165   (* AddexperiencePartyGlobal(S:Name,S:Area) *)
	| _, 166   (* SetNumTimesTalkedTo(I:Num) *)
	| _, 167   (* StartMovie(S:ResRef) *)
	| _, 168   (* Interact(O:Object) *)
	| _, 169   (* DestroyItem(S:ResRef) *)
	| _, 170   (* RevealAreaOnMap(S:ResRef) *)
	| _, 171   (* GiveGoldForce(I:Amount) *)
	| _, 172   (* ChangeTileState(O:Tile,I:StateBoolean) *)
	| _, 173   (* AddJournalEntry(I:Entry,I:TypeJourType) *)
	| _, 174   (* EquipRanged() *)
	| _, 175   (* SetLeavePartyDialogFile() *)
	| _, 176   (* EscapeAreaDestroy(I:Delay) *)
	| _, 177   (* TriggerActivation(O:Object,I:StateBoolean) *)
	| _, 178   (* BreakInstants() *)
	...
	| _, 195   (* Lock(O:Object) *)
	| _, 196   (* Unlock(O:Object) *)
	| _, 197   (* MoveGlobal(S:Area,O:Object,P:Point) *)
	| _, 198   (* StartDialogueNoSet(O:Object) *)
	| _, 199   (* TextScreen(S:TextList) *)
	| _, 200   (* RandomWalkContinuous() *)
	| _, 201   (* DetectSecretDoor(O:Object) *)
	| _, 202   (* FadeToColor(P:Point,I:Blue) *)
	| _, 203   (* FadeFromColor(P:Point,I:Blue) *)
	| _, 204   (* TakePartyItemNum(S:ResRef,I:Num) *)
	| _, 207   (* MoveToPointNoInterrupt(P:Point) *)
	| _, 208   (* MoveToObjectNoInterrupt(O:Object) *)
	| _, 209   (* SpawnPtActivate(O:Object) *)
	| _, 210   (* SpawnPtDeactivate(O:Object) *)
	| _, 211   (* SpawnPtSpawn(O:Object) *)
	| _, 212   (* GlobalShout(I:ID) *)
	| _, 213   (* StaticStart(O:Object) *)
	| _, 214   (* StaticStop(O:Object) *)
	...
	| _, 228   (* CreateCreatureImpassable(S:NewObject,P:Location,I:Face) *)
	| _, 229   (* FaceObject(O:Object) *)
	| _, 230   (* RestParty() *)
	| _, 231   (* CreateCreatureDoor(S:NewObject,P:Location,I:Face) *)
	...
	| _, 235   (* SetQuestDone(I:STRREF) *)
	| _, 236   (* StorePartyLocations() *)
	| _, 237   (* RestorePartyLocations() *)
	| _, 238   (* CreateCreatureOffScreen(S:ResRef,I:Face) *)
	...
	| _, 246   (* CreateCreatureAtLocation(S:GLOBAL,S:Area,S:ResRef) *)
	...
	| _, 251   (* HideAreaOnMap(S:ResRef) *)
	| _, 252   (* CreateCreatureObjectOffset(S:ResRef,O:Object,P:Offset) *)
	| _, 253   (* ContainerEnable(O:Object,I:BoolBOOLEAN) *)
	| _, 254   (* ScreenShake(P:Point,I:Duration) *)
	| _, 255   (* AddGlobals(S:Name,S:Name2) *)
	...
	| _, 263   (* EraseJournalEntry(I:STRREF) *)
	| _, 264   (* CopyGroundPilesTo(S:ResRef,P:Location) *)
	...
	| _, 292   (* DisplayStringHeadOwner(S:Item,I:STRREF) *)
	...
	| _, 299   (* BanterBlockTime(I:Time) *)
	| _, 300   (* BanterBlockFlag(I:StateBoolean) *)
	| _, 301   (* AmbientActivate(O:Object,I:StateBOOLEAN) *)
	| _, 302   (* AttachTransitionToDoor(S:GLOBAL,O:Object) *)
	| _, 303   (* DeathMatchPositionGlobal(S:Areaname,P:Dest,I:Player) *)
	| _, 304   (* DeathMatchPositionArea(S:Areaname,P:Dest,I:Player) *)
	| _, 305   (* DeathMatchPositionLocal(S:Areaname,P:Dest,I:Player) *)
	| _, 306   (* ApplyDamagePercent(O:Object,I:Amount,I:TypeDMGTYPE) *)
	| _, 307   (* SG(S:Name,I:Num) *)
	| _, 308   (* AddMapNote(P:Position,I:StringRef) *)
	| _, 309   (* DemoEnd() *)
	| _, 310   (* MoveGlobalsTo(S:FromArea,S:ToArea,P:Location) *)
	| _, 311   (* DisplayStringWait(O:Object,I:StrRef) *)
	| _, 312   (* StateOverrideTime(I:Time) *)
	| _, 313   (* StateOverrideFlag(I:StateBoolean) *)
	| _, 314   (* SetRestEncounterProbabilityDay(I:Prob) *)
	| _, 315   (* SetRestEncounterProbabilityNight(I:Prob) *)
	| _, 316   (* SoundActivate(O:Object,I:StateBOOLEAN) *)
	| _, 317   (* PlaySong(I:Song) *)
	...
	| _, 321   (* SetAreaRestFlag(I:CanRest) *)
	| _, 322   (* FakeEffectExpiryCheck(O:Object,I:Ticks) *)
	| _, 323   (* CreateCreatureImpassableAllowOverlap(S:NewObject,P:Location,I:Face) *)
	| _, 324   (* SetBeenInPartyFlags() *)
	| _, 325   (* GoToStartScreen() *)
	| _, 326   (* ExitPocketPlane() *)
	| _, 327   (* AddXP2DA(S:Column) *)
	| _, 328   (* RemoveMapNote(P:Position,I:STRREF) *)
	| _, 329   (* TriggerWalkTo(O:Object) *)
	| _, 330   (* AddAreaType(I:TypeAREATYPE) *)
	| _, 331   (* RemoveAreaType(I:TypeAREATYPE) *)
	| _, 332   (* AddAreaFlag(I:TypeAREAFLAG) *)
	| _, 333   (* RemoveAreaFlag(I:TypeAREAFLAG) *)
	...
	| _, 338   (* SetCutSceneLite(I:BOOLBOOLEAN) *)
	| _, 339   (* SwingOnce() *)
	| _, 340   (* StaticSequence(O:Object,I:Sequence) *)
	| _, 341   (* StaticPalette(S:Palette,O:Object) *)
	| _, 342   (* DisplayStringHeadDead(O:Object,I:StrRef) *)
	| _, 343   (* MoveToExpansion() *)
	| _, 344   (* StartRainNow() *)
	| _, 345   (* SetSequence(I:SequenceSEQ) *)
	| _, 346   (* DisplayStringNoNameHead(O:Object,I:StrRef) *)
	| _, 347   (* SetEncounterProbability(S:FromArea,S:ToArea,I:Probability) *)
...
let invalid_for_ict1 (al: string list) = false
