...
global option to your \ttref{TP2} file. \\
  ...
  or & \DEFINE{README} file \Slist &
       ...
       they are checked for existance in the order they're provided, and the first available
...
\end{verbatim}
  ...
  or & \DEFINE{MOVE} fromFile toFile ... &
       You may specify as many fromFile-toFile pairs as you like. Each
       fromFile is moved to its associated toFile. Wildcard support will
       be added on request.
       ...
       MAKE!BIFF after MOVE. Do not use the --make-biff command-line argument).
       \\
  ...
  or & \DEFINE{CLEAR!EVERYTHING} & Runs CLEAR!MEMORY, CLEAR!ARRAY, CLEAR!CODES and
      CLEAR!INLINED. \\
...
P Q R 3 <br>
  ...
  or & \DEFINE{AT!EXIT} commandToRun \Ob \t{EXACT} \Oe &
    ...
    underlying operating system. If \verb+EXACT+ is specified, \verb+/+
...
or & \DEFINE{REFACTOR!BAF!TRIGGER} \ttref{optcase} \ttref{optexact} String String &
  ...
  broken (like REPLACE!TEXTUALLY would do if you are substituting one trigger with
  ...
  \item the resulting file will have messed up whitespace, so remember that if
    you plan on using REPLACE!TEXTUALLY later (the file will work correctly
	in-game). Applying \ttref{COMPILE!BAF!TO!BCS}
	and \ttref{DECOMPILE!BCS!TO!BAF} in tandem will fix this issue.
  \item the searching regexp must match exactly against a single trigger
    (so \verb+REFACTOR!BAF!TRIGGER ~A() B()~ ~C()~+ or
	\verb+REFACTOR!BAF!TRIGGER ~A(~ ~C() A(~+ will not work). The substituting
	string can contain multiple triggers.
  \end{itemize}
\\
or & \DEFINE{REFACTOR!D!TRIGGER} \ttref{optcase} \ttref{optexact} String String &
  Like \ttref{REFACTOR!BAF!TRIGGER}, but for .D files. The same caveats apply.
\\
...
or & \DEFINE{TRA!ENTRY!EXISTS} ( \ttref{String} \ttref{String} \Slist ) &
	...
	otherwie it's looked for into the listed TRA files.
\\
or & \DEFSYN{IS!SILENT} &
...
\\
The vertex points are seperated by their X & Y values
...
\begin{itemize}
\item SET ab_RT_Vx_X_0 -- Point #0 X value
\item SET ab_RT_Vx_Y_0 -- Point #0 Y value
\end{itemize}
...
\subsection{\DEFINE{fj_add_are_structure}}
Adds a structure to an area file. All variables are zero or blank by default unless otherwise indicated. Fields designated by an asterisk are typically required; all others are optional. Kept for compatibility reasons, new code should use \ttref{fj_are_structure} instead.
This is a PATCH function.

Universal structure variables:
\begin{itemize}
\item SPRINT \verb+fj_structure_type+ to the type of area structure to be added (actor, region, spawn, entrance, container, itm, ambient, variable, door, animation, note or projectile)*;
\item RET \verb+fj_return_offset+ returns the position of the structure in the area file;
\end{itemize}
Actor structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the actor's name*;
\item SET \verb+fj_loc_x+ to the starting X (horizontal) coordinate*;
\item SET \verb+fj_loc_y+ to the starting Y (vertical) coordinate*;
\item SET \verb+fj_dest_x+ to the destination X coordinate (normally the same as fj_loc_x)*;
\item SET \verb+fj_dest_y+ to the destination Y coordinate (normally the same as fj_loc_y)*;
\item SET \verb+fj_loading+ to whether the .cre file is loaded (0=attached, 1=loaded, default 1 - currently does not support attached CREs);
\item SET \verb+fj_spawned+ to whether the creature has been spawned (0=no, 1=yes);
\item SET \verb+fj_animation+ to the actor's animation number (from animate.ids, though the engine uses the animation set on the .cre file);
\item SET \verb+fj_orientation+ to the facing direction (0-15 where 0=south, 4=west, 8=north, 12=east)*;
\item SET \verb+fj_expiry+ to the actor removal timer in absolute ticks (default -1 to avoid removal);
\item SET \verb+fj_wander_dist_actor+ to the actor's random walk distance limit;
\item SET \verb+fj_mvmt_dist_actor+ to the actor's movement distance limit;
\item SET \verb+fj_schedule+ to the hourly appearance schedule (bits 0-23, default -1 or always);
\item SET \verb+fj_num_talked+ to the NumTimesTalkedTo (in .sav files);
\item SPRINT \verb+fj_dlg_resref+ to the actor's dialogue file (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_override+ to the actor's override script (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_general+ to the actor's general script (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_class+ to the actor's class script (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_race+ to the actor's race script (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_default+ to the actor's default script (normally obtained from .cre files);
\item SPRINT \verb+fj_bcs_specific+ to the actor's specific script (normally obtained from .cre files);
\item SPRINT \verb+fj_cre_resref+ to the actor's resource reference (creature filename)*;
\end{itemize}
Region structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the region's name*;
\item SET \verb+fj_type+ to the region type (0=trap, 1=info, 2=travel)*;
\item SET \verb+fj_box_left+ to the leftmost X coordinate of the region's bounding box*;
\item SET \verb+fj_box_top+ to the topmost Y coordinate of the region's bounding box*;
\item SET \verb+fj_box_right+ to the rightmost X coordinate of the region's bounding box*;
\item SET \verb+fj_box_bottom+ to the bottommost Y coordinate of the region's bounding box*;
\item SET \verb+fj_cursor_idx+ to the region's mouse cursor index (from cursors.bam)*;
\item SPRINT \verb+fj_destination_area+ to the destination area resource reference (for travel regions);
\item SPRINT \verb+fj_destination_name+ to the entrance name in the destination area (for travel regions);
\item SET \verb+fj_flags+ to the bitwise region flags;
\item SET \verb+fj_info_point_strref+ to the information text string reference (for info points, default -1);
\item SET \verb+fj_trap_detect+ to the trap detection difficulty percentage;
\item SET \verb+fj_trap_remove+ to the trap removal difficulty percentage;
\item SET \verb+fj_trap_active+ to whether the region is trapped (0=no, 1=yes);
\item SET \verb+fj_trap_status+ to whether the trap is detected (0=no, 1=yes);
\item SET \verb+fj_loc_x+ to the trap launch X coordinate*;
\item SET \verb+fj_loc_y+ to the trap launch Y coordinate*;
\item SPRINT \verb+fj_key_resref+ to the filename of the region's key;
\item SPRINT \verb+fj_reg_script+ to the region script;
\item SET \verb+fj_alt_x+ to the activation point X coordinate;
\item SET \verb+fj_alt_y+ to the activation point Y coordinate;
\item SET \verb+fj_vertex_0+ to (X coordinate + (Y coordinate << 16)) for each vertex pair*;
\end{itemize}
Spawn structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the spawn point's name*;
\item SET \verb+fj_loc_x+ to the spawning X coordinate*;
\item SET \verb+fj_loc_y+ to the spawning Y coordinate*;
\item SPRINT \verb+fj_cre_resref0+ to the resource reference of each creature spawned (0-9)*;
\item SET \verb+fj_spawn_num+ to the count of spawn creatures*;
\item SET \verb+fj_base_num+ to the base number of creatures to spawn (encounter difficulty)*;
\item SET \verb+fj_delay+ to the number of seconds between spawning (default 10);
\item SET \verb+fj_method+ to the spawn method;
\item SET \verb+fj_duration+ to the creature duration (default 1000);
\item SET \verb+fj_wander_dist_spawn+ to the creature's random walk distance limit (default 1000);
\item SET \verb+fj_mvmt_dist_spawn+ to the creature's movement distance limit (default 1000);
\item SET \verb+fj_max_num+ to the maximum number of creatures to spawn*;
\item SET \verb+fj_enable+ to the spawn point status (0=inactive, 1=active, default 1);
\item SET \verb+fj_schedule+ to the hourly appearance schedule (bits 0-23, default -1 or always);
\item SET \verb+fj_day_prob+ to the spawn point daytime probability (default 100);
\item SET \verb+fj_night_prob+ to the spawn point nighttime probability (default 100);
\end{itemize}
Entrance structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the entrance's name*;
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_orientation+ to the facing direction (0-15 where 0=south, 4=west, 8=north, 12=east)*;
\end{itemize}
Container structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the container's name*;
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_type+ to the container type (1=bag, 2=chest, 3=drawer, 4=pile, 5=table, 6=shelf, 7=altar, 8=nonvisible, 9=spellbook, 10=body, 11=barrel, 12=crate)*;
\item SET \verb+fj_lock_diff+ to the lock difficulty (default 100);
\item SET \verb+fj_flags+ to the bitwise container flags (bit0=locked, bit3=trap resets, bit5=disabled);
\item SET \verb+fj_trap_detect+ to the trap detection difficulty percentage;
\item SET \verb+fj_trap_remove_diff+ to the trap removal difficulty percentage (default 100);
\item SET \verb+fj_trap_active+ to whether the container is trapped (0=no, 1=yes);
\item SET \verb+fj_trap_status+ to whether the trap is detected (0=no, 1=yes);
\item SET \verb+fj_trap_loc_x+ to the trap launch X coordinate*;
\item SET \verb+fj_trap_loc_y+ to the trap launch Y coordinate*;
\item SET \verb+fj_box_left+ to the leftmost X coordinate of the trap's bounding box;
\item SET \verb+fj_box_top+ to the topmost Y coordinate of the trap's bounding box;
\item SET \verb+fj_box_right+ to the rightmost X coordinate of the trap's bounding box;
\item SET \verb+fj_box_bottom+ to the bottommost Y coordinate of the trap's bounding box;
\item SPRINT \verb+fj_trap_script+ to the trap's script;
\item SET \verb+fj_vertex_0+ to (X coordinate + (Y coordinate << 16)) for each vertex pair*;
\item SPRINT \verb+fj_key_resref+ to the filename of the container's key;
\item SET \verb+fj_lockpick_strref+ to the lockpick string reference (default -1);
\end{itemize}
Item structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the item's resource reference (filename)*;
\item SET \verb+fj_itm_expiry+ to the item expiration time;
\item SET \verb+fj_charge0+ to the charges of the 1st ability (item quantity for stackables);
\item SET \verb+fj_charge1+ to the charges of the 2nd ability;
\item SET \verb+fj_charge1+ to the charges of the 3rd ability;
\item SET \verb+fj_flags+ to the bitwise item flags (bit0=identified, bit1=unstealable, bit2=stolen, bit3=undroppable);
\end{itemize}
Ambient structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the ambient's name*;
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_radius+ to the sound radius (default 500);
\item SET \verb+fj_loc_z+ to the Z coordinate (height);
\item SET \verb+fj_volume+ to the volume percentage (default 80);
\item SPRINT \verb+fj_wav_resref0+ to the resource reference of each sound (0-9)*;
\item SET \verb+fj_sound_num+ to the number of sounds*;
\item SET \verb+fj_delay+ to the base interval in seconds between sounds from this ambient list;
\item SET \verb+fj_variation+ to the base deviation from the base interval;
\item SET \verb+fj_schedule+ to the hourly appearance schedule (bits 0-23, default -1 or always);
\item SET \verb+fj_flags+ to the bitwise ambient flags*;
\end{itemize}
Variable structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the variable's name*;
\item SET \verb+fj_variable_value+ to the variable's value*;
\end{itemize}
Door structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the door's name*;
\item SPRINT \verb+fj_door_wed_id+ to the door ID linked to the .wed file*;
\item SET \verb+fj_flags+ to the bitwise door flags*;
\item SET \verb+fj_open_box_left+ to the leftmost X coordinate of the open door's bounding box*;
\item SET \verb+fj_open_box_top+ to the topmost Y coordinate of the open door's bounding box*;
\item SET \verb+fj_open_box_right+ to the rightmost X coordinate of the open door's bounding box*;
\item SET \verb+fj_open_box_bottom+ to the bottommost Y coordinate of the open door's bounding box*;
\item SET \verb+fj_closed_box_left+ to the leftmost X coordinate of the closed door's bounding box*;
\item SET \verb+fj_closed_box_top+ to the topmost Y coordinate of the closed door's bounding box*;
\item SET \verb+fj_closed_box_right+ to the rightmost X coordinate of the closed door's bounding box*;
\item SET \verb+fj_closed_box_bottom+ to the bottommost Y coordinate of the closed door's bounding box*;
\item SET \verb+fj_door_open_vert_0+ to (X coordinate + (Y coordinate << 16)) for each vertex pair of the open door*;
\item SET \verb+fj_door_closed_vert_0+ to (X coordinate + (Y coordinate << 16)) for each vertex pair of the closed door*;
\item SET \verb+fj_cell_open_vert_0+ to (X coordinate + (Y coordinate << 16)) for each impeded search map cell of the open door*;
\item SET \verb+fj_cell_closed_vert_0+ to (X coordinate + (Y coordinate << 16)) for each impeded search map cell of the closed door*;
\item SPRINT \verb+fj_door_open_wav+ to the door open sound;
\item SPRINT \verb+fj_door_close_wav+ to the door closed sound;
\item SET \verb+fj_cursor_idx+ to the door's mouse cursor index (from cursors.bam)*;
\item SET \verb+fj_trap_detect+ to the trap detection difficulty percentage;
\item SET \verb+fj_trap_remove+ to the trap removal difficulty percentage;
\item SET \verb+fj_trap_active+ to whether the door is trapped (0=no, 1=yes);
\item SET \verb+fj_trap_status+ to whether the trap is detected (0=no, 1=yes);
\item SET \verb+fj_trap_loc_x+ to the trap launch X coordinate*;
\item SET \verb+fj_trap_loc_y+ to the trap launch Y coordinate*;
\item SPRINT \verb+fj_key_resref+ to the filename of the door's key;
\item SPRINT \verb+fj_door_script+ to the door script;
\item SET \verb+fj_detect_diff+ to the detection difficulty (for secret doors);
\item SET \verb+fj_locked_diff+ to the lock difficulty;
\item SET \verb+fj_open_loc_x+ to the X coordinate for toggling the door's open state*;
\item SET \verb+fj_open_loc_y+ to the Y coordinate for toggling the door's open state*;
\item SET \verb+fj_closed_loc_x+ to the X coordinate for toggling the door's closed state*;
\item SET \verb+fj_closed_loc_y+ to the Y coordinate for toggling the door's closed state*;
\item SET \verb+fj_lockpick_strref+ to the lockpick string reference (default -1);
\item SPRINT \verb+fj_travel_trigger+ to the travel region name*;
\item SET \verb+fj_dlg_strref+ to the dialogue string reference (default -1);
\item SPRINT \verb+fj_dlg_resref+ to the door's dialogue file;
\end{itemize}
Animation structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the animation name*;
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_schedule+ to the hourly appearance schedule (bits 0-23, default -1 or always);
\item SPRINT \verb+fj_bam_resref+ to the animation resource reference (filename)*;
\item SET \verb+fj_bam_seq+ to the BAM sequence number;
\item SET \verb+fj_bam_frame+ to the BAM frame number;
\item SET \verb+fj_flags+ to the bitwise animation flags*;
\item SET \verb+fj_loc_z+ to the height;
\item SET \verb+fj_transparent+ to the BAM transparency;
\item SET \verb+fj_init_frame+ to the starting frame;
\item SET \verb+fj_loop_chance+ to the chance of looping;
\item SET \verb+fj_skip_cycles+ to start delay in frames;
\item SPRINT \verb+fj_bmp_resref+ to the palette bitmap;
\end{itemize}
Map note structure variables:
\begin{itemize}
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_note_strref+ to the note string reference (default -1)*;
\item SPRINT \verb+fj_note_text+ to the note text (PST only);
\item SET \verb+fj_strref_loc+ to the strref location (0=external, 1=dialog.tlk, default 1);
\item SET \verb+fj_color+ to the map marker color (0-7);
\item SET \verb+fj_note_id+ to the note ID;
\end{itemize}
Projectile trap structure variables:
\begin{itemize}
\item SPRINT \verb+fj_name+ to the projectile filename*;
\item SET \verb+fj_eff_off+ to the effect block offset;
\item SET \verb+fj_eff_size+ to the effect block size;
\item SET \verb+fj_missile_num+ to the missile.ids reference (projectl.ids - 1)*;
\item SET \verb+fj_frequency to the explosion length (in frames);
\item SET \verb+fj_duration+ to the number of explosions;
\item SET \verb+fj_loc_x+ to the X coordinate*;
\item SET \verb+fj_loc_y+ to the Y coordinate*;
\item SET \verb+fj_loc_z+ to the height;
\item SET \verb+fj_target+ to the target ID;
\end{itemize}
...
\begin{verbatim}
  ...
  END
\end{verbatim}
Example 4: add a container to an area
\begin{verbatim}
  LPF fj_add_are_structure
    INT_VAR
...
\begin{itemize}
\item OUTER_SPRINT \verb+areName+ to the resource reference of the area to add (like "ar0700");
\item OUTER_SPRINT \verb+strName+ to the descriptive name of the area (like "Waukeen's Promenade");
\item OUTER_SPRINT \verb+strDesc+ to the area description that will show up when hovering the cursor over the area on the worldmap;
\item OUTER_SET \verb+mapIcon+ to the area's map icon index (from mapicons.bam);
\item OUTER_SET \verb+xCoord+ to the area's X (east-west) coordinate;
\item OUTER_SET \verb+yCoord+ to the area's Y (north-south) coordinate;
\item OUTER_SET \verb+tTime+ to the area's travel time in hours * 4 (so 2 = 8 hours);
\item OUTER_SET \verb+inclSv+ to 1 to patch saved game worldmaps as well as the master worldmap;
...
OUTER_SET tTime   = 0              // travel time *4, so two means eight hours
OUTER_SET inclSv  = 1              // include saved games bool: 1 means yes and 0 means no
...
\item wisp (sends documentation!)
\item Miloch (sends documentation!)
\end{itemize}
and additionally
\begin{itemize}
