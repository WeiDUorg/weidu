...
global option to your \ttref{TP2} file. \\
  is & \DEFINE{AUTO!TRA} path \Ob subdir \Oe &
       ...
       \t{EXTEND!TOP} \ttref{TP2 Action}s. It automatically loads
...
\end{verbatim}
  ...
  or & \DEFSYN{EXTEND!BOTTOM} existingBCS newFile \ttref{patch} \Slist
    \Ob \t{USING} traName \Slist \Oe \Ob \ttref{when} \Slist \Oe &
    As \t{EXTEND!TOP}, but the newFile file is put at the bottom of the
  ...
  or & \DEFINE{EXTEND!TOP!REGEXP} existingBCSregexp newFile \ttref{patch} \Slist
    \Ob \t{USING} traName \Slist \Oe \Ob \ttref{when} \Slist \Oe &
    As \t{EXTEND!TOP}, but the newFile file is put at the top of the
...
\DEFINE{optNoBackup} & &
  ...
  or & \t{-} & If you put a \t{-} here, WeiDU will {\em not} copy the file,
           ...
           then use it for \t{EXTEND!BOTTOM} or whatever).
...
when a \ttref{COPY}, \ttref{COPY!EXISTING}, \t{APPEND},
\ttref{APPEND!COL}, \t{EXTEND!TOP}, \t{EXTEND!BOTTOM},
...
checked against the \ttref{when} clauses separately. \\
  ...
  \ttref{COPY!EXISTING!REGEXP}, \ttref{COPY!RANDOM}, \t{APPEND},
  \ttref{APPEND!COL}, \t{EXTEND!TOP}, \t{EXTEND!BOTTOM},
...
or & \DEFINE{VARIABLE!IS!IN!ARRAY} \ttref{String} & % Not actually a String
Returns true if the variable String is set, as per
...
Now, writing the code to \t{EXTEND!TOP} this little script into every area
...
\t{EXTEND!TOP} and \t{EXTEND!BOTTOM}, and Westley obliged.  (My penance
for that request is writing the docs you are reading right now.)

So now with Wes' help, I could \t{EXTEND!TOP} to every area in 1 fell
