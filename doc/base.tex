...
\DEFINE{--dry-run} & Simulate installation without changing game files,
mod backup files, uninstall metadata, \t{WeiDU.log}, or running shell
commands. WeiDU still reads files, evaluates TP2 logic, and writes normal
console/debug/audit output. Because dry-run suppresses writes rather than
creating a virtual filesystem, later actions read the real filesystem and may
not follow all runtime branches exactly. For broader branch exploration,
combine with \ttref{--continue}. \\
\DEFINE{--allow-outside-gamedir} & Suppress warnings for operations whose
target path resolves outside the game directory.
By default, WeiDU warns and audits these operations without blocking them,
to preserve compatibility with large mod stacks (BGT or EET installations). \\
\DEFINE{--warn-shell} & Print a visible warning whenever a mod executes a
shell command. \\
\DEFINE{--audit-log} \t{X} & Write audit entries to path \t{X} instead of
the default \t{<gamedir>/weidu-audit.log}. \\
\DEFINE{--audit-log-json} \t{X} & Write audit entries in NDJSON format to
path \t{X} (one JSON object per line). \\
\DEFINE{--no-audit-log} & Disable audit logging. \\
\DEFINE{--strict-path-risk} & Block operations that target HIGH-RISK paths
outside the game directory (opt-in strict mode).
Examples of blocked targets: On \t{Windows}, drive roots (\t{C:\\}),
system directories (\t{C:\\Windows\\*}), and program files (\t{C:\\Program Files\\*}, \t{C:\\ProgramData\\*}).
On \t{macOS}, the filesystem root (\t{/}), system directories (\t{/System/*}),
and OS binaries (\t{/usr/*}, \t{/bin/*}). On \t{Linux}, the root (\t{/}),
configuration (\t{/etc/*}), OS binaries (\t{/usr/*}, \t{/bin/*}), and system
libraries (\t{/lib/*}, \t{/boot/*}). \\
\DEFINE{--require-sha256} & Fail component installation when a SHA-256 hash
cannot be computed (instead of allowing MD5 fallback). \\
\end{tabular}
...
\end{verbatim}
  ...
  or & \t{\textbf{APPEND}}\label{APPEND (TP2)} \ttref{optNoBackup} filename
      newText \ttref{when} \Slist \Ob \t{KEEP_CRLF} \Oe  &
...
\end{verbatim}
  ...
  or & \t{\textbf{EXTEND_TOP}}\label{EXTEND_TOP (TP2)} existingBCS
  ...
  or & \t{\textbf{EXTEND_BOTTOM}}\label{EXTEND_BOTTOM (TP2)} existingBCS
    newFile \ttref{patch} \Slist \Ob \t{USING} traName \Slist \Oe
    \Ob \ttref{when} \Slist \Oe &
...
or & \DEFINE{ACTION_REINCLUDE} \t{\ttref{String}} \Slist &
  ...
  or & \t{<<<<<<<<} fileName fileBody \t{>>>>>>>>}\label{inlined} &
    ...
    on fileName (such as \ttref{FILE_EXISTS}, \ttref{FILE_MD5}, \ttref{FILE_SHA256} or
...
or & \DEFINE{REPLACE_EVALUATE} \ttref{optcase} findRegexp \t{BEGIN}
  ...
  replaceRegexp. Note that \ttref{REPLACE_EVALUATE} defines up to 200
  ...
  list (e.g., \ttref{SAY} or \ttref{WRITE_ASCII}) are ignored:
...
files can have the same MD5 checksum. In any
event, the discovered checksum is printed to the log. If the file does not
exist, it evaluates to 0.

\emph{MD5 is NOT collision-resistant! Do not use without a real reason.} \\
or & \DEFINE{FILE_SHA256} fileName sha256sum & Evaluates to 1 if the file exists
and has the given SHA-256 checksum. Evaluates to 0 otherwise. Two different
files are exceptionally unlikely to have the same SHA-256 checksum. In any
event, the discovered checksum is printed to the log. If the file does not
exist, or SHA-256 support is unavailable at runtime, it evaluates to 0.
Method selection is based on the detected OS and tried in this order:
\begin{itemize}
\item \t{Windows} (\t{Sys.os_type = "Win32"}):
  \t{certutil -hashfile fileName SHA256}, then
  \t{powershell -NoProfile -Command "(Get-FileHash -Path fileName -Algorithm SHA256).Hash"}
\item \t{Unix-like} (Linux/macOS):
  \t{sha256sum fileName}, then \t{shasum -a 256 fileName}
\end{itemize}
If all methods fail or are unavailable, \ttref{FILE_SHA256} evaluates to 0. \\
