...
SAVE_DIRECTORY evaluates to the directory in which the current game type stores its saved games. This is "./save" on non-BGEE games. On Windows BGEE games this is "%PATH%\Baldur's Gate - Enhanced Edition\save", where PATH is the user's personal directory as read from the Windows registry. On OS X BGEE games this is "$HOME/Library/Containers/com.beamdog.baldursgateenhancededition/Data/Documents/Baldur's Gate - Enhanced Edition/save", where HOME is the user's home directory, as retrieved by uid from the user database.
\end{verbatim}
