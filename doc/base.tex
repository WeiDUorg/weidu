...
or & \DEFINE{REPLACE_BCS_BLOCK} \Ob \t{EVALUATE_BUFFER} \Oe \ttref{optcase}
  ...
  \\
or & \DEFINE{REPLACE_BCS_BLOCK_REGEXP} oldFile newFile
     \Ob \t{ON_MISMATCH} \ttref{patch} \Slist \t{END} \Oe &
  Replaces every non-overlapping segment of the current file that matches the
  \ttref{regexp} read from oldFile with the replacement template read from
  newFile. Matching is case-sensitive. Both files are read as textual,
  compiled-\ttref{BCS} fragments; they are not compiled from \ttref{BAF}, and
  no script header or footer is removed.

  The replacement template may contain \verb+\0+ for the complete match and
  \verb+\1+ through \verb+\9+ for the corresponding \verb+\(...\)+ group in
  oldFile. For example, a compiled-BCS pattern containing
  \verb+\([0-9]+\) 0 0 0 0"GLOBALTimer" "" AC+ can preserve the matched
  integer by using \verb+\1 0 0 0 0"GLOBALTimer" "" AC+ in newFile.

  Variables in the oldFile and newFile \emph{filenames} are evaluated before
  the files are loaded. The contents of the files are otherwise used verbatim.

  If the regular expression is not found and the \t{ON_MISMATCH} section is
  present, those patches are run. If it is not found and \t{ON_MISMATCH} is
  absent, a warning is printed and the current file remains unchanged.

  You may use \DEFINE{R_B_B_RE} as a synonym for
  \ttref{REPLACE_BCS_BLOCK_REGEXP}.
  \\
