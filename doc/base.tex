...
\end{verbatim}
  ...
  or & \DEFINE{CREATE} type \Ob \t{VERSION} version \Oe resref \Ob \ttref{patch} \Slist \Oe &
      ...
      current game, but can be overridden with the \t{VERSION} option,
      which takes a string specifying the desired
      version. \ttref{CREATE} supports the file types \t{ARE},
...
\verb+bg2=soa, tob, iwd2, pst, bg1, totsc, iwd=iwd1, how, tolm=totlm, tutu, tutu_totsc, bgt, ca, iwd_in_bg2, bgee, bg2ee+.
...
\begin{itemize}
\item INT_VAR \verb+check_globals+ to whether the function should loop through global effects on items (also known as equipping effects) and spells if set to 1 (default is 1). Creature effects are all global effects, so this variable will always be considered to be one when this function is run on a creature.
...
\item STR_VAR \verb+charset_table+ to the name of an associative array where the keys are the names of your language directories and the corresponding values are the character sets used by the respective language.
