...
unusabilities
  ...
  or & \DEFINE{ADD!SECTYPE} sectypeName removalString &
	...
	\\
  or & \DEFINE{ADD!AREA!FLAG} areaFlagName &
	Adds a new (guaranteed-unique) area flag to areaflag.ids, and sets
  the areaFlagName variable to the index of the bit the flag is associated
  with: the flag can be set with 
  \verb+WRITE_SHORT 0x48 THIS | (1 << areaFlagName)+.
	
	
	
	If  areaflag.ids already contains areaflagname, the variable is set to
	the flag's current identifier and no other operation is performed.
  
  
  
  Also note that only 8 area flags can be added to an unmodded installation
  of BG2; as such, don't add a new area flag type unless you really need it
  (most likely, another modder is already using a flag with a similar name,
  and you can share the flag between your mods without issue).
	\\
...
name (either by using this function or other code of your devising). If you
provide 'base', it will attempt to create the file name from base (similarly
to the Tutu algorithm); for example, \verb+base = spwi117+ might yield
\verb+filename = __pwi117.%extension%+; if this file already exists
(or you didn't provide \verb+base+), then WeiDU will attempt sequentially
with \verb+__000000.%extension%+,  \verb+__000001.%extension%+, up to
\verb+__999999.%extension%+, at which point the searched name space is exausted
and the patch will fail. This allows you to create (at least) a million files
per extension, which is enough for the foreseeable future.
\begin{itemize}
\item STR_VAR extension to the desired extension (required).
\item STR_VAR base to the resource of the cloned resource (optional).
\item RET filename the currently unallocated file name.
\end{itemize}
\\
