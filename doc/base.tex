...
\DEFINE{Component Flag} &&
  ...
  or & \DEFINE{METADATA} \ttref{String} &
      ...
      \t{METADATA} may be used multiple times per component, but only
      the last instance is associated with the component.\\ \\
...
except \verb+bg2=soa+ will detect SoA-based Tutu games in addition to
SoA games, and \verb+tob+ will detect ToB-based Tutu games in addition
to ToB and BGT games.  As such, a mod that requires Throne of Bhaal's
engine enhancements could use
\verb+REQUIRE_PREDICATE ENGINE_IS ~tob~+. Additionally, \t{ENGINE!IS}
can be used to identify \verb+gemrb+. \\
