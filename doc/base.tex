...
\end{verbatim}
  ...
  or & \DEFINE{WITH!TRA} traName \Slist \t{BEGIN} \ttref{TP2 Action} \Slist \t{END} &
    Creates a new \t{TRA} scope copied from the surrounding scope, loads all traName files
    (variable substitution is done; the file can be inlined) and evaluates the \t{TP2 Action}s.
    When \ttref{WITH!TRA} exits the \t{TRA} scope is restored to what it was. \\
...
or & \DEFINE{PATCH!WITH!TRA} traName \Slist \t{BEGIN} \ttref{patch} \Slist \t{END} &
    Creates a new \t{TRA} scope copied from the surrounding scope, loads all traName files
    (variable substitution is done; the file can be inlined) and evaluates the \t{patch}s.
    When \ttref{PATCH!WITH!TRA} exits the \t{TRA} scope is restored to what it was. \\
