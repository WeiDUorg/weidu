...
\end{verbatim}
  ...
  or & \DEFINE{MOVE} fromFile toFile ... &
       ...
       (exception: if you MOVE for biffing purposes, it is save to call
...
unusabilities
  ...
  or & \DEFINE{ADD!SPELL} newSPLfile type level idsName \ttref{patch} \Slist
	...
	is removed from spell.ids, and then the action proceeds as usual. You still
	need to make sure that the old version is unavailable to players.
	\\
...
or & \DEFIN{TRA!ENTRY!EXISTS} ( \ttref{String} \ttref{String} \Slist ) &
...
or & \DEFINE{INSTALL!ORDER} modTp2Name1 modComponent1 BEFORE modTp2Name2 modComponent2 &
