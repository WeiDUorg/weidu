...
This is a PATCH macro and function.
\begin{itemize}
\item SPRINT AREA_RT_NAME   name to assign new region
\item SET AREA_RT_TYPE  type of trigger -- 0=proximity;1=info;2=travel
\item SET AREA_RT_BBOX_LOW_X bounding box low x value LEFT
\item SET AREA_RT_BBOX_LOW_Y bounding box low y value TOP
\item SET AREA_RT_BBOX_HIGH_X bounding box high x value RIGHT
\item SET AREA_RT_BBOX_HIGH_Y bounding box high y value BOTTOM
\item SET AREA_RT_VERTEX_PAIRS number of x y coordinate pairs making up new region
\item SET AREA_RT_FLAGS flag values set in bit format from right to left 1 turns on 0 turns off
\item SET AREA_RT_TRAP_DET_DIFF trap detection difficulty percentage
\item SET AREA_RT_TRAP_REM_DIFF trap removal difficulty percentage
\item SET AREA_RT_TRAP_IS_SET Is the region trapped? 0=no;1=yes
\item SET AREA_RT_TRAP_DETECTED Is the region trap already detected? 0=no;1=yes
\item SET AREA_RT_LAUNCH_POINT_X Main use point x value
\item SET AREA_RT_LAUNCH_POINT_Y Main use point y value
\item SPRINT AREA_RT_KEY_ITEM key item reference if one is required
\item SPRINT AREA_RT_REGION_SCRIPT region script
\item SET AREA_RT_ALT_USE_POINT_X alternate use point x value
\item SET AREA_RT_ALT_USE_POINT_Y alternate use point y value
...
Example: For the X values: AREA_RT_VERTEX_X0, AREA_RT_VERTEX_X1, AREA_RT_VERTEX_X2
         For the Y values: AREA_RT_VERTEX_Y0, AREA_RT_VERTEX_Y1, AREA_RT_VERTEX_Y2
\item SET AREA_RT_VERTEX_X0 Point #0 X value
\item SET AREA_RT_VERTEX_Y0 Point #0 Y value

\\
if travel trigger set these
\item SET AREA_RT_CURSOR_INDEX a point in cursors.bam -- used for travel triggers
\item SPRINT AREA_RT_DEST_AREA destination area for travel triggers
\item SPRINT AREA_RT_ENTRANCE_NAME entrance name in destincation area for travel triggers

\\
if info trigger set these
\item SPRINT AREA_RT_INFO_TEXT text to be displayed for info triggers

\\
if using on PST set this if you need to
\item SPRINT AREA_RT_DIALOG_FILE dialog file if the region can talk in pst\end{itemize}
...
\item {\tt REMOVE!2DA!ROW}.
\item {\tt GET!FILE!ARRAY}.
\item {\tt GET!DIRECTORY!ARRAY}.
