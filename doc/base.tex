...
or & \DEFSYN{ADD!STORE!ITEM} \Ob \t{+} \Oe itemName \Ob position \Oe
  ...
  the default behaviour. See the \ttref{ADD!STORE!ITEM} tutorial for
  more information.\\
...
or & \DEFINE{IF!EXISTS} & True only if the file exists. For \ttref{COPY}, this check is done with \ttref{FILE!EXISTS}. For \ttref{COPY!EXISTING}, \ttref{COPY!EXISTING!REGEXP}, \ttref{COPY!RANDOM}, \t{APPEND} and \ttref{APPEND!COL} this check is done with \ttref{FILE!EXISTS!IN!GAME}. This when clause does nothing for \ttref{COPY!ALL!GAM!FILES}.
\\
...
\verb+bg2=soa, tob, iwd2, pst, bg1, totsc, iwd=iwd1, how, totlm, tutu, tutu_totsc, bgt, ca, iwd_in_bg2, bgee, bg2ee, eet, iwdee, sod+.
...
\verb+ADD_SPELL_CFEFFECT+: adds a casting-feature effect to a spell.
This is a PATCH macro and function.
\begin{list}{}{}
\item Syntax is the same as that of \verb+ADD_SPELL_EFFECT+
\end{list}
...
\verb+USER_DIRECTORY+ evaluates to the directory in which user files are kept. This is the game path on non-BG: EE games. On EE-type games, if the file engine.lua exists, WeiDU will attempt to read the game-specific directory from it. If the engine.lua does not exist or does not contain the necessary information, WeiDU will use the default paths. On Windows BG: EE games this is "\%PATH\%\backslash Baldur's Gate - Enhanced Edition". On OS X BG: EE games this is "\$HOME/Documents/Baldur's Gate - Enhanced Edition". On Windows BGII: EE games this is "\%PATH\%\backslash Baldur's Gate II - Enhanced Edition". On OS X BGII: EE games this is "\$HOME/Documents/Baldur's Gate II - Enhanced Edition".
