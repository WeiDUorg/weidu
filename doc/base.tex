...
global option to your \ttref{TP2} file. \\
  ...
  or & \DEFSYN{UNINSTALL!ORDER} String \Slist &
		...
		The operations are STRSET (uninstall STRSET and ALTER!TLK), MOVE
  ...
  or & \DEFINE{AUTO!EVAL!STRINGS} &
    If used, an EVALUATE!BUFFER is implied everywhere possible. In particular,
...
\end{verbatim}
  ...
  or & \DEFINE{MOVE} fromFile toFile ... \verb+(+ \ttref{directory-file-regexp} \verb+)+ toFile ... &
       You may specify as many fromFile-toFile pairs as you like. Each
	   ...
	   \verb+MOVE+ also supports the \ttref{directory-file-regexp} construct
...
\end{verbatim}
       ...
       As such, do your AT!NOW, then do your COPY, then do your MOVEs
  ...
  or & \DEFINE{DELETE} \ttref{optGlob} file ... &
        Removes one or more files from the file system. If optGlob is not specified, the
        file is moved to the backup folder; if optGlob is specified, the file is definitively
        removed.
...
or & \DEFINE{PATCH!IF} \ttref{value} \Ob \t{THEN} \Oe \t{BEGIN}
  ...
  \\
  or & \DEFINE{PATCH!MATCH}\Slist WITH \Ob \ttref{value} ...
    \Ob WHEN \ttref{value} \Oe
...
-1 if not found. If provided, value specifies from what point of string2
the searching should begin (instead of STRING!LENGTH string2).
\\
...
-1 if not found. If provided, value specifies from what point of the string
the searching should begin (instead of BUFFER!LENGTH).
\\
...
This is a PATCH function. For all integer arguments, a negative value results in no change to that field and the default value is -1. The argument \verb+entry_name+ is required.
\begin{itemize}
...
\item STR_VAR \verb+entry_name+ to the name of the entrance point to be altered. This argument is required.
\end{itemize}
\\
...
This is a PATCH function. For all integer arguments, a negative value results in no change and the default value is -1. In the case of flags, a value of 1 will set the flag and a value of 0 will unset it. For all string arguments except \verb+trig_name+, the default value is the string "same", which results in no change to that field. The argument \verb+trig_name+ is required.
\begin{itemize}
...
\item INT_VAR \verb+trap_detectable+ to the new value of the flag known as \t{trap is detectable} (bit 3).
...
\item STR_VAR \verb+trig_name+ to the name of the region to be patched. This argument is required.
\item STR_VAR \verb+dest_area+ to the resref of the new destination area.
\item STR_VAR \verb+ent_name+ to the new entrance name of the destination area.
...
This is a PATCH function. All integer arguments default to the value -1 and a negative value results in no change to the corresponding field. All string arguments except \verb+actor_name+ default to the string "same", which results in no change to the corresponding field. The argument \verb+actor_name+ is required.
\begin{itemize}
\item INT_VAR \verb+x_coord+ to the new x coordinate.
\item INT_VAR \verb+y_coord+ to the new y coordinate.
\item INT_VAR \verb+orient+ to the new facing direction.
\item STR_VAR \verb+actor_name+ to the name of the actor to be patched. This argument is required.
...
\verb+ALTER_AREA_CONTAINER+: patch \ahref{\url{http://iesdp.gibberlings3.net/file_formats/ie_formats/are_v1.htm#formAREAV1_0_Container}}{area containers}, but not their contents. This is a PATCH function. All integer arguments default to -1 and negative values result in no change to the corresponding field. In the case of flags, a value of 1 will set the flag and a value of 0 will unset it. All string arguments except \verb+cont_name+ default to the string "same", which results in no change to the corresponding field. The argument \verb+cont_name+ is required.
\begin{itemize}
\item INT_VAR \verb+cont_type+ to the new type of the container. %changed this
...
\item INT_VAR \verb+lockpick_strref+ to the new strref of the lockpick string. %changed this
\item INT_VAR \verb+lock_difficulty+ to the new difficulty of the lock. %changed this
\item INT_VAR \verb+trap_detect+ to the new difficulty of trap detection. %changed this
\item INT_VAR \verb+trap_remove+ to the new difficulty of trap removal. %changed this
...
\item STR_VAR \verb+cont_name+ to the name of the container to be patched. This argument is required.
\item STR_VAR \verb+cont_script+ to the resref of the new container script.
\item STR_VAR \verb+cont_key+ to the resref of the new container key.
\end{itemize}
\\

\verb+ALTER_AREA_DOOR+: patch \ahref{\url{http://iesdp.gibberlings3.net/file_formats/ie_formats/are_v1.htm#formAREAV1_0_Door}}{area doors.} This is a PATCH function. All integer arguments default to -1 and negative values result in no change to the corresponding field. All string arguments except \verb+door_name+ default to the string "same", which results in no change to the corresponding field. The argument \verb+door_name+ is required.
\begin{itemize}
\item INT_VAR \verb+door_icon+ to the new cursor index.
\item INT_VAR \verb+trap_detect+ to the new difficulty of trap detection.
\item INT_VAR \verb+trap_remove+ to the new difficulty of trap removal.
...
\item INT_VAR \verb+lock_difficulty+ to the new difficulty of the lock. %changed this
\item STR_VAR \verb+door_name+ to the name of the door to be patched. This argument is required.
...
SAVE_DIRECTORY the directory in which the current game type stores its saved games. This is "./save" on non-BGEE games and something like
"%USERPROFILE%/Baldur's Gate - Enhanced Edition/save" on BGEE games, where the absolute path is read from the Windows registry.
\end{verbatim}
