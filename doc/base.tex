...
or & \DEFINE{DECOMPILE!AND!PATCH} BEGIN \ttref{patch} \Slist 
  ...
  (and can indiscriminately patch BCS and DLG files).
  \begin{verbatim}
  
  \end{verbatim}
  \verb+DECOMPILE!AND!PATCH!END+ is used instead of \verb+END+ to aid in
  detecting mods that use \verb+DECOMPILE!A!TO!B+ without the matching
  \verb+COMPILE!B!TO!A+; you will be able to use \verb+END+ in
  a future version of WeiDU.
\\
