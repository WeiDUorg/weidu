...
or [R]einstall the currently installed components.

If the user chooses to install a group, then its components will be installed,
and ALL other components of the mod will be uninstalled if already installed.

Components in ALWAYS_ASK will always be asked for individually (unless the user
chose to [S]kip all) and will never be installed or uninstalled because of
QUICK_MENU. If a component is in both ALWAYS_ASK and a group the mod will
refuse to install.

Caveats:
\begin{itemize}
\item this command will not work with ASK_EVERY_COMPONENT or GROUP (the mod will
refuse to install). If your mod requires to use GROUP in tandem with QUICK_MENU I
might make the two commands work together.
...
install), must not be in any group or in ALWAYS_ASK (or your mod will refuse to
install) and will always be installed (unless the user chooses [S]kip all).
This means that your mod's components will always be grouped together (you can't
install a component from your mod, then a second mod, and then other components
of your mod).
\item ALWAYS_ASK (and the listed failure points) are completely ignored when
the components are specified in the command line (--uninstall, --yes,
--reinstall, --force-install, --force-uninstall).
\end{itemize}
