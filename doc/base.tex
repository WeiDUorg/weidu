...
\t{Setup-MyMod.exe}) in the current directory from the running executable.
Sibling setup files are never executed or used as update sources. A sibling
that declares a newer \t{WeiDU} version is left untouched and must be run
directly if it is trusted. \\
...
\\
\DEFINE{--allow-file-root} \t{X} & Authorize TP2-initiated reads and writes
within installation root \t{X} (cumulative). WeiDU automatically authorizes
the selected game directory and the validated Enhanced Edition user-data
directory. With \ttref{--nogame}, it authorizes the working directory.
TP2 source directories and directories supplied through \ttref{--search} or
\ttref{--search-ids} receive read authority. Existing path components are
resolved before each access, so a symbolic link cannot escape an authorized
root. Access elsewhere is refused; use this option for operator-approved
multi-game layouts such as BGT or EET. \\
