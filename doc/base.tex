...
\ahref{\url{http://iesdp.gibberlings3.net/file_formats/ie_formats/bcs.htm}}{IESDP}.
...
is & \t{IF} \ttref{script trigger} ... \t{THEN} \ttref{script response} ... \t{END} &
  ...
  If the \ttref{script trigger}(s) are true, the \ttref{script
  response}(s) are evaluated. \\
...
is & \Ob !\Oe\ttref{Infinity Engine trigger} &
  ...
  \ahref{\url{http://iesdp.gibberlings3.net/scripting/triggers/index.htm}}{IESDP}.
  A preceding exclamation mark negates the trigger. \\

or & \t{TriggerOverride}(\ttref{Infinity Engine object},\Ob !\Oe\ttref{Infinity Engine trigger}) &
...
is & \t{RESPONSE} \t{#}weight \ttref{Infinity Engine action} ... &
  ...
  \ahref{\url{http://iesdp.gibberlings3.net/scripting/actions/index.htm}}{IESDP}.
...
\DEFINE{--tlkin} \t{X}	& Semi-deprecated. \emph{Do not use this.} Use {\tt X} as {\tt DIALOG.}\ttref{TLK} (instead
of looking for \t{DIALOG.TLK} in the game directory). \\
\DEFINE{--ftlkin} \t{X} & Semi-deprecated. \emph{Do not use this.} Use {\tt X} as {\tt DIALOGF.}\ttref{TLK} (instead
...
\DEFINE{--tlkout} \t{X} & Semi-deprecated. \emph{Do not use this.} Redirect the output dialog to X. If any strings were added to or changed in the loaded \t{DIALOG.TLK}, emit \t{X} as an updated version that reflects those changes. Many operations (e.g., compiling \t{D} files, \ttref{--tlkmerge}, \ttref{STRING!SET}) can add to or modify \t{DIALOG.TLK}. If this option is not provided, the output dialog is the same as the input dialog. \\
\DEFINE{--ftlkout} \t{X} & Semi-deprecated. \emph{Do not use this.} Redirect the output dialogf to X. If any strings were added to or changed in the
...
or & \DEFINE{DEFINE!ACTION!FUNCTION} \ttref{String}
     ...
     \t{END} &
  Define an action function. Function definitions support the
  additional keyword \t{UNDEFINED}, which may be used instead of a
  value or string on the right of an equality sign when declaring
  variables in the \t{INT!VAR} or \t{STR!VAR} sections. A variable
  declared as \t{UNDEFINED} will be undefined in the function's scope,
  regardless of its status outside the function, unless a value for
  the variable is passed when the function is launched. Refer to the
  \ttref{Functions} section for additional information about using
  functions. \\
or & \DEFINE{DEFINE!PATCH!FUNCTION} \ttref{String}
     ...
     \t{END} &
  Define a patch function. Function definitions support the additional
  keyword \t{UNDEFINED}, which may be used instead of a value or
  string on the right of an equality sign when declaring variables in
  the \t{INT!VAR} or \t{STR!VAR} sections. A variable declared as
  \t{UNDEFINED} will be undefined in the function's scope, regardless
  of its status outside the function, unless a value for the variable
  is passed when the function is launched. Refer to the
  \ttref{Functions} section for additional information about using
  functions. \\
...
WMP_NORTH_LINKS (0x38 4 0x50 4 0x54 4 0xd8)
WMP_WEST_LINKS (0x38 4 0x58 4 0x5c 4 0xd8)
WMP_SOUTH_LINKS (0x38 4 0x60 4 0x64 4 0xd8)
WMP_EAST_LINKS (0x38 4 0x68 4 0x6c 4 0xd8)
\end{verbatim}
...
\verb+bg2=soa, tob, iwd2, pst, bg1, totsc, iwd=iwd1, how, totlm, tutu, tutu_totsc, bgt, ca, iwd_in_bg2, bgee, bg2ee, eet, iwdee+.
...
\verb+bg1, totsc, bg2=soa, tob, pst, iwd=iwd1, how, totlm, iwd2, ca+. In
...
\verb+ALTER_AREA_DOOR+: patch \ahref{\url{http://iesdp.gibberlings3.net/file_formats/ie_formats/are_v1.htm#formAREAV1_0_Door}}{area doors.} This is a PATCH function. All integer variables, except \verb+string_unlock+ and \verb+string_speaker+, default to -1 and negative values result in no change to the corresponding field. The variables \verb+string_unlock+ and \verb+string_speaker+ instead default to 99999999. In the case of flags, a value of 1 will set the flag and a value of 0 will unset it. All string variables except \verb+door_name+ default to the string "same", which results in no change to the corresponding field. The variable \verb+door_name+ is required.
\begin{itemize}
...
\item INT_VAR \verb+match_savebonus+ to the saving trow bonus effects to be matched (default is 11). Since saving throws can range into negative values, this variable can match all the way down to10.
...
\DEFINE{\verb+HANDLE_CHARSETS+}: runtime-convert \t{TRA} files into \t{UTF-8} in a safe and easy manner. This is an ACTION function.
...
\verb+EE_LANGUAGE+ evaluates to the Enhanced Edition language directory the user has selected, for example, \verb+en_US+. This variable is undefined on non-EE games.
...
\item Argent77 (sends tp2 functions)
\end{itemize}
