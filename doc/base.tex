...
This is a PATCH function. For all integer variables, a negative value results in no change to that field and the default value is -1. The variable \verb+entry_name+ is required.
\begin{itemize}
...
\item STR_VAR \verb+entry_name+ to the name of the entrance point to be altered. This variable is required.
\end{itemize}
\\
...
This is a PATCH function. For all integer variables, a negative value results in no change and the default value is -1. In the case of flags, a value of 1 will set the flag and a value of 0 will unset it. For all string variables except \verb+trigger_name+, the default value is the string "same", which results in no change to that field. The variable \verb+trigger_name+ is required.
\begin{itemize}
...
\item STR_VAR \verb+trigger_name+ to the name of the region to be patched. This variable is required.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section mainly serves the purpose of rightfully acknowledging the
efforts of those who have contributed functionality to WeiDU without
cluttering the documentation of said functionality with kudos (however well-deserved
they may be).
Please refer to the \ttref{Special Thanks} for additional recognitions.

Many thanks to
\begin{itemize}
\item CamDawg, for contributing
  \begin{itemize}
    \item ALTER!AREA!ENTRANCE
    \item ALTER!AREA!REGION
    \item ALTER!AREA!ACTOR
    \item ALTER!AREA!CONTAINER
    \item ALTER!AREA!DOOR
    \item ALTER!ITEM!EFFECT
    \item ALTER!ITEM!HEADER
    \item DELETE!ITEM!HEADER
    \item ALTER!SPELL!EFFECT
    \item ALTER!SPELL!HEADER
    \item DELETE!SPELL!HEADER
  \end{itemize}
\item Gort, for contributing
  \begin{itemize}
    \item DELETE!SPELL!EFFECT
    \item DELETE!ITEM!EFFECT
    \item DELETE!ITEM!EQEFFECT
    \item DELETE!CRE!EFFECT
    \item ITEM!EFFECT!TO!SPELL
    \item ADD!SPELL!EFFECT
    \item ADD!ITEM!EFFECT
    \item ADD!ITEM!EQEFFECT
    \item ADD!CRE!EFFECT
    \item DELETE!CRE!ITEM
    \item DELETE!STORE!ITEM
    \item DELETE!AREA!ITEM
    \item REPLACE!STORE!ITEM
    \item REPLACE!AREA!ITEM
    \item ADD!AREA!ITEM
    \item ADD!CRE!ITEM!FLAGS
    \item REMOVE!CRE!ITEM!FLAGS
    \item SET!CRE!ITEM!FLAGS
    \item READ!SOUNDSET
    \item WRITE!SOUNDSET
  \end{itemize}
\item Miloch, for contributing
  \begin{itemize}
    \item T-CRE!EFF!V1
  \end{itemize}
\item Nythrun, for contributing
  \begin{itemize}
    \item FJ!CRE!VALIDITY
    \item FJ!CRE!REINDEX
    \item FJ!CRE!EFF!V2
    \item fj\_are\_structure
  \end{itemize}
\item plainab, for contributing
  \begin{itemize}
    \item ADD!AREA!REGION!TRIGGER
  \end{itemize}
\item SConrad, for contributing
  \begin{itemize}
    \item sc#addWmpAre
  \end{itemize}
\end{itemize}
