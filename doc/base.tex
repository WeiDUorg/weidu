...
{tt \DEFINE{--game-by-type} X} & tries to guess the game path by reading
the registry; X can be one of BG1, BG2, PST, IWD1 or IWD2.
\\
...
or & \DEFINE{REFACTOR!BAF!TRIGGER} \ttref{optcase} \ttref{optexact} String String &
  Synonim of \ttref{REFACTOR!TRIGGER}.
\\
...
\verb+ADD_SPELL_EFFECT+: adds an extended effect to a spell. All variables except probability1 and insert_point are 0 by default.
This is a PATCH macro and function.
\begin{itemize}
...
This is a PATCH macro and function.
\begin{itemize}
\item SPRINT item_to_add to the item you want to add 
\item SET container_to_add_to to the number of the container the item should be added to. The count starts at 1.
...
\item INT_VAR do_reindex to 0 if you don't want the creature file to be reindexed. Defaults to 1.
\item INT_VAR do_eff to 0 if you don't want EFFv1 <-> EFFv2 conversion to be applied. Defaults to 1.
...
\item STR_VAR base to a string unique to your mod and current file (optional).
\item RET filename the currently unallocated file name (sans the extension).
\end{itemize}
\\
...
the GPL'd \t{ocaml, batteries included} library and the BSD'd \t{Elkhound} program.
